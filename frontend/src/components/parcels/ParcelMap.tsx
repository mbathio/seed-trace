...
const ParcelMap = ({ 
    ...
    try {
      ...
      if (editMode && map.current) {
        map.current.on('click', (e) => {
          ...
          // Create new marker
          const marker = new mapboxgl.Marker({ color: '#34D399', draggable: true })
  ...
  useEffect(() => {
    ...
    parcels.forEach(parcel => {
      const color = parcel.id === selectedParcelId 
    ...
    <Card className="overflow-hidden">
      {mapboxToken === 'REPLACE_WITH_ACTUAL_MAPBOX_TOKEN' && (
        <div className="p-4 bg-amber-50 border-b border-amber-200">
          <Label htmlFor="mapbox-token">Entrez votre token Mapbox pour activer la carte:</Label>
          ...
          <p className="text-xs text-amber-600 mt-1">
            Obtenir un token sur <a href="https://mapbox.com" target="_blank" rel="noopener noreferrer" className="underline">mapbox.com</a>
          </p>
        </div>
      )}
