...
import { Calendar, CheckCircle2, CloudRain, Droplets, Leaf, LineChart, Thermometer, Timer, Trash2, XCircle } from "lucide-react";
import { 
  Production, 
  MOCK_PRODUCTIONS, 
  MOCK_SEED_LOTS, 
...
const ProductionDetail = () => {
  ...
  const lot = production ? MOCK_SEED_LOTS.find(l => l.id === production.lotId) : undefined;
  ...
  const handleWeatherSubmit = () => {
    // Validate form
    if (!newWeatherData.date || !newWeatherData.temperature || !newWeatherData.rainfall || !newWeatherData.humidity) {
      toast.error("Veuillez remplir tous les champs obligatoires");
      return;
    }
  ...
  const getActivityTypeLabel = (type: ActivityType) => {
    switch (type) {
      case "soil_preparation": return "Préparation du sol";
      case "sowing": return "Semis";
      case "fertilization": return "Fertilisation";
      case "irrigation": return "Irrigation";
      case "weeding": return "Désherbage";
      case "pest_control": return "Contrôle des nuisibles";
      ...
      <div className="flex">
        ...
        <main className="flex-1 p-6 ml-64">
          <div className="mb-6 flex justify-between items-start">
            <div>
              <h1 className="text-2xl font-bold">Production #{production.id}</h1>
              <p className="text-gray-600">
                Lot: {production.lotId} • Parcelle: {parcel.name || `#${parcel.id}`}
              </p>
            </div>
            <div>
          ...
          <div className="grid grid-cols-1 md:grid-cols-3 gap-6 mb-6">
            <Card>
              <CardHeader className="pb-2">
                <CardTitle className="text-sm font-medium">Date de semis</CardTitle>
              </CardHeader>
              <CardContent>
                <div className="flex items-center">
                  <Calendar className="h-5 w-5 mr-2 text-isra-green" />
                  <span className="text-xl font-bold">
                    {production.sowingDate ? production.sowingDate.toLocaleDateString() : "Non définie"}
                  </span>
                </div>
              </CardContent>
            </Card>
            ...
            <Card>
              <CardHeader className="pb-2">
                <CardTitle className="text-sm font-medium">
                  {production.status === "completed" ? "Date de récolte" : "Récolte prévue"}
                </CardTitle>
              </CardHeader>
              <CardContent>
                <div className="flex items-center">
            ...
            <Card>
              <CardHeader className="pb-2">
                <CardTitle className="text-sm font-medium">
                  {production.status === "completed" ? "Rendement obtenu" : "Production planifiée"}
                </CardTitle>
              </CardHeader>
              <CardContent>
                <div className="flex items-center">
                  ...
                  <CardContent>
                    <div className="space-y-4">
                      ...
                      <div className="flex justify-between py-2 border-b">
                        ...
                        <span className="font-medium">{parcel.name || `#${parcel.id}`}</span>
                      </div>
                      <div className="flex justify-between py-2 border-b">
                        <span className="text-gray-600">Date de semis</span>
                        <span className="font-medium">{production.sowingDate ? production.sowingDate.toLocaleDateString() : "Non définie"}</span>
                      </div>
                      <div className="flex justify-between py-2 border-b">
                      ...
                      <div className="flex justify-between py-2 border-b">
                        <span className="text-gray-600">Production planifiée</span>
                        <span className="font-medium">{production.plannedQuantity || 0} kg</span>
                      </div>
                      <div className="flex justify-between py-2 border-b">
                        <span className="text-gray-600">Rendement obtenu</span>
                        <span className="font-medium">
                          {production.yield ? `${production.yield} kg` : "Non disponible"}
                        </span>
                      </div>
                      ...
                      <div className="space-y-4">
                        <div>
                          <div className="flex justify-between mb-2">
                            ...
                            <span className="font-medium">
                              {production.status === "planned" ? "10%" : 
                               production.status === "in-progress" ? "60%" : "100%"}
                            </span>
                          </div>
                          <Progress 
                            value={production.status === "planned" ? 10 : 
                                  production.status === "in-progress" ? 60 : 100} 
                            className="h-2"
                          />
                        </div>
                        ...
                        <div className="pt-4">
                          ...
                          <div className="space-y-2">
                            ...
                            <div className="flex items-center">
                              ...
                              <span className={production.status === "planned" ? "text-gray-500" : ""}>
                                Semis
                              </span>
                            </div>
                            <div className="flex items-center">
                              ...
                              <span className={production.status !== "completed" ? "text-gray-500" : ""}>
                                Récolte
                              </span>
                            </div>
                            <div className="flex items-center">
                  ...
                  <Card>
                    ...
                    <CardContent>
                      <div className="space-y-3">
                        <div className="flex justify-between py-2 border-b">
                          <span className="text-gray-600">Identifiant du lot</span>
                        ...
                        <div className="flex justify-between py-2 border-b">
                          ...
                          <Badge className={
                            ...
                            lot.level === 'G3' ? 'bg-green-500' :
                            lot.level === 'G4' ? 'bg-teal-500' :
                            lot.level === 'R1' ? 'bg-purple-500' : 'bg-pink-500'
                          }>
                            {lot.level}
                          </Badge>
                        </div>
                        <div className="flex justify-between py-2">
                          <span className="text-gray-600">Quantité initiale</span>
                ...
                <div className="lg:col-span-2">
                  <Card>
                    <CardHeader>
                      ...
                      <CardDescription>
                        Calendrier des opérations effectuées sur cette production
                      </CardDescription>
                    </CardHeader>
                    <CardContent>
                          ...
                          <div className="space-y-8 relative ml-10">
                            {production.activities.map((activity, index) => (
                              <div key={index} className="relative">
                                <div className={`absolute -left-14 mt-1.5 h-4 w-4 rounded-full border border-white ${
                                  activity.type === "soil_preparation" ? "bg-isra-brown" :
                                  activity.type === "sowing" ? "bg-isra-green" : 
                                  activity.type === "harvest" ? "bg-amber-500" : "bg-blue-500"
                                }`}></div>
                                <div className="bg-white p-4 rounded-lg shadow-sm">
                                  <div className="flex justify-between">
                                    <h4 className="font-bold">{getActivityTypeLabel(activity.type)}</h4>
                                  ...
                                  {activity.inputs && activity.inputs.length > 0 && (
                                    <div className="mt-3">
                                      <h5 className="text-sm font-medium text-gray-600">Intrants utilisés:</h5>
                                      <div className="grid grid-cols-2 gap-2 mt-1">
                                        {activity.inputs.map((input, idx) => (
                                          <div key={idx} className="text-xs bg-gray-50 p-2 rounded">
                                            <strong>{input.name}</strong>: {input.quantity} {input.unit}
                                          </div>
                                        ))}
                                      </div>
                                    </div>
                      ...
                      <div className="space-y-4">
                        <div className="space-y-2">
                          ...
                          >
                            ...
                            <SelectContent>
                              <SelectItem value="soil_preparation">Préparation du sol</SelectItem>
                              ...
                              <SelectItem value="pest_control">Contrôle des nuisibles</SelectItem>
                        ...
                        <div className="space-y-2">
                          <Label htmlFor="activity-notes">Notes additionnelles</Label>
                          <Textarea 
                            id="activity-notes" 
                      ...
                      {production.issues && production.issues.length > 0 ? (
                        <div className="grid gap-4">
                          {production.issues.map((issue, index) => (
                            <div key={index} className="bg-white p-4 rounded-lg shadow-sm border-l-4 border-amber-500">
                              <div className="flex justify-between items-start">
                                <div>
                                  <div className="flex items-center">
                                    <h4 className="font-bold">{getIssueTypeLabel(issue.type)}</h4>
                                ...
                                <Badge className={issue.resolved ? "bg-green-500" : "bg-red-500"}>
                              ...
                              <div className="mt-3 pt-3 border-t border-gray-100">
                                <h5 className="text-sm font-medium text-gray-600">Actions prises:</h5>
                      ...
                      <div className="space-y-4">
                        <div className="space-y-2">
                          ...
                          <Input 
                            ...
                            type="date"
                            value={newIssue.date}
                            onChange={(e) => handleIssueChange("date", e.target.value)}
                          />
                        </div>
                        ...
                        <div className="space-y-2">
                          ...
                          <Select 
                            value={newIssue.type} 
                            onValueChange={(value) => handleIssueChange("type", value)}
                          >
                        ...
                        <div className="space-y-2">
                          ...
                          <Select 
                            ...
                            onValueChange={(value) => handleIssueChange("severity", value)}
                          >
                        ...
                        <div className="space-y-2">
                          ...
                          <Textarea 
                            ...
                            onChange={(e) => handleIssueChange("actions", e.target.value)}
                          />
                        </div>
                        ...
                        <div className="flex items-center space-x-2">
                          <input 
                            ...
                            onChange={(e) => handleIssueChange("resolved", e.target.checked)}
                            className="rounded border-gray-300 text-isra-green focus:ring-isra-green"
                          />
                          <Label htmlFor="issue-resolved">Problème résolu</Label>
                        </div>
                      </div>
                    </CardContent>
                    <CardFooter className="flex justify-end">
                ...
                <div className="lg:col-span-2">
                  <Card>
                    <CardHeader>
                      ...
                      <CardDescription>
                        Suivi des conditions météo pendant la période de production
                      </CardDescription>
                    </CardHeader>
                    <CardContent>
                          ...
                          <table className="w-full">
                            <thead>
                              <tr className="border-b">
                                ...
                                <th className="text-left py-3 px-4">Température (°C)</th>
                                <th className="text-left py-3 px-4">Précipitations (mm)</th>
                                <th className="text-left py-3 px-4">Humidité (%)</th>
                              </tr>
                            </thead>
                            <tbody>
                      ...
                      <div className="space-y-4">
                        <div className="space-y-2">
                          ...
                          <Input 
                            ...
                            onChange={(e) => handleWeatherChange("date", e.target.value)}
                          />
                        </div>
                        
                        <div className="space-y-2">
                          <Label htmlFor="weather-temperature">Température (°C)</Label>
                          <Input 
                        ...
                        <div className="space-y-2">
                          <Label htmlFor="weather-rainfall">Précipitations (mm)</Label>
                          <Input 
                            id="weather-rainfall" 
                            type="number" 
                            step="0.1"
