...
import { UserRole, MOCK_USERS, MOCK_MULTIPLIERS, MOCK_PARCELS, MOCK_SEED_LOTS, MOCK_VARIETIES } from "@/utils/seedTypes";
...
import { MapPin, Phone, Mail, Award, Calendar, FileText, Crop } from "lucide-react";
          ...
          <div className="flex items-center gap-4 mb-6">
            ...
            <div>
              ...
              <p className="text-gray-600">
                ID: {multiplier.id} • {multiplier.location.lat.toFixed(4)}, {multiplier.location.lng.toFixed(4)}
              </p>
            </div>
            <div className="ml-auto flex gap-2">
              <Dialog open={isContractDialogOpen} onOpenChange={setIsContractDialogOpen}>
                <DialogContent className="sm:max-w-[500px]">
                  <DialogHeader>
                    <DialogTitle>Nouveau Contrat avec {multiplier.name}</DialogTitle>
                    <DialogDescription>
                  ...
                  <div className="grid gap-4 py-4">
                    ...
                    <div>
                      ...
                      <Select 
                        ...
                        onValueChange={(value) => handleContractChange("varietyId", value)}
                      >
                    ...
                    <div className="grid grid-cols-2 gap-4">
                      <div>
                        ...
                        <Select 
                          ...
                          onValueChange={(value: any) => handleContractChange("seedLevel", value)}
                        >
                      ...
                      <div>
                        ...
                        <Input 
                          ...
                          onChange={(e) => handleContractChange("quantity", e.target.value)}
                        />
                      </div>
                    </div>
                  </div>
          ...
          <Tabs defaultValue="overview" value={activeTab} onValueChange={setActiveTab} className="mb-6">
                ...
                <Card>
                  ...
                  <CardContent className="space-y-4">
                    <div className="flex items-center space-x-3">
                      <Badge className={multiplier.status === "active" ? "bg-green-500" : "bg-gray-500"}>
                        {multiplier.status === "active" ? "Actif" : "Inactif"}
                      </Badge>
                    ...
                    <div className="space-y-2">
                      ...
                      <div className="flex items-center gap-2">
                        ...
                        <span>{multiplier.yearsExperience} ans d'expérience</span>
                      </div>
                    </div>
                  </CardContent>
                </Card>
                  ...
                  <CardContent>
                    <div className="space-y-4">
                      {multiplier.history && (
                        <>
                          <div>
                            <h3 className="font-medium mb-2">Production récente</h3>
                            <div className="space-y-2">
                              {multiplier.history.map((record, index) => {
                                const variety = MOCK_VARIETIES.find(v => v.id === record.varietyId);
                                return (
                                  <div key={index} className="flex justify-between items-center">
                                    <div>
                                      <span className="font-medium">{record.season} {record.year}</span>
                                    ...
                                    <div>
                                      <Badge className={
                                        record.qualityScore >= 90 ? "bg-green-500" : 
                                        record.qualityScore >= 80 ? "bg-amber-500" : "bg-red-500"
                                      }>
                  ...
                  <Card className="md:col-span-3">
                    ...
                    <CardContent>
                      <div className="space-y-3">
                        {multiplier.contracts.filter(c => c.status === 'active').map(contract => {
                          const variety = MOCK_VARIETIES.find(v => v.id === contract.varietyId);
                          return (
                            <div key={contract.id} className="flex justify-between items-center border-b border-gray-100 pb-3">
                              <div>
                                <div className="flex items-center gap-2">
                                  <FileText className="h-4 w-4 text-gray-500" />
                                  <span className="font-medium">Contrat #{contract.id}</span>
                                </div>
                                <div className="text-sm text-gray-500">
                                  {variety?.name} • Niveau {contract.seedLevel} • {contract.quantity} kg
                                </div>
                              </div>
                    ...
                    <CardContent>
                      <div className="space-y-3">
                        {multiplierParcels.map(parcel => (
                          <div key={parcel.id} className="flex justify-between items-center border-b border-gray-100 pb-3">
                            <div>
                              <div className="flex items-center gap-2">
                                <Crop className="h-4 w-4 text-gray-500" />
                                <span className="font-medium">{parcel.name || `Parcelle #${parcel.id}`}</span>
                              </div>
                              <div className="text-sm text-gray-500">
                            ...
                            <div className="text-right">
                              <Badge className={
                              ...
                              }>
                                {
                                  ...
                                  parcel.status === 'in-use' ? 'En utilisation' : 'En jachère'
                                }
                              </Badge>
                            </div>
                          </div>
            ...
            <TabsContent value="contracts" className="space-y-4 mt-4">
              <div className="flex justify-between mb-4">
                <h2 className="text-xl font-semibold">Contrats de multiplication</h2>
                <Button 
                  className="bg-isra-green hover:bg-isra-green-dark"
                  onClick={() => setIsContractDialogOpen(true)}
                >
                ...
                <CardContent className="p-6">
                  {multiplier.contracts && multiplier.contracts.length > 0 ? (
                          ...
                          {multiplier.contracts.map(contract => {
                            ...
                            return (
                              <tr key={contract.id} className="border-b">
                                ...
                                <td className="py-3 px-4">{contract.seedLevel}</td>
                                ...
                                <td className="py-3 px-4">
                                  <Badge className={
                                    ...
                                    contract.status === 'completed' ? 'bg-blue-500' : 'bg-red-500'
                                  }>
                                    {
                                      contract.status === 'draft' ? 'Brouillon' :
                                      contract.status === 'active' ? 'Actif' :
                                      contract.status === 'completed' ? 'Terminé' : 'Résilié'
                                    }
                                  </Badge>
                                </td>
                                <td className="py-3 px-4">
                    ...
                    <div className="text-center py-10">
                      <p className="text-gray-500">Aucun contrat pour ce multiplicateur</p>
                      <Button 
            ...
            <TabsContent value="parcels" className="space-y-4 mt-4">
              <div className="flex justify-between mb-4">
                <h2 className="text-xl font-semibold">Parcelles du multiplicateur</h2>
              ...
              {multiplierParcels.length > 0 ? (
                    ...
                    <Card key={parcel.id}>
                      ...
                      <CardContent>
                        ...
                        {parcel.previousCrops && parcel.previousCrops.length > 0 && (
                          <div className="mt-4">
                            <h4 className="text-sm font-medium mb-2">Cultures précédentes:</h4>
                ...
                <div className="text-center py-10">
                  <p className="text-gray-500">Aucune parcelle associée à ce multiplicateur</p>
              ...
              <Card>
                ...
                <CardContent>
                  {multiplier.history && multiplier.history.length > 0 ? (
                      ...
                      <table className="w-full text-sm">
                        <thead>
                          <tr className="border-b">
                            ...
                            <th className="text-left py-3 px-4">Niveau</th>
                            <th className="text-left py-3 px-4">Quantité</th>
                            <th className="text-left py-3 px-4">Score qualité</th>
                          </tr>
                        </thead>
                        <tbody>
                          {multiplier.history.map((record, index) => {
                            ...
                            return (
                              <tr key={index} className="border-b">
                                <td className="py-3 px-4">{record.season} {record.year}</td>
                                <td className="py-3 px-4">{variety?.name}</td>
                    ...
                    <div className="text-center py-10">
                      <p className="text-gray-500">Aucun historique de production disponible</p>
                    </div>
                  )}
                </CardContent>
              </Card>
